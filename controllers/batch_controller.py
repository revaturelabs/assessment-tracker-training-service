...
def route(ans, ins):
    batch_data = ans.model(
        'Batch Data', {
            "name":
            fields.String(default="Reston-7152021",
                          description='Name of the batch'),
            "trainingTrack":
            fields.String(default="Python, Java, Automation",
                          description='Tech Stack for the batch'),
            "startDate":
            fields.Integer(default="1625843430",
                           description='Epoch time in seconds'),
            "endDate":
            fields.Integer(default="1639008000",
                           description='Epoch time in seconds')
        })
    ...
    class CreateBatch(Resource):
        ...
        def post(self):
            ...
            try:
                body = request.json
                batch = Batch(body["name"], body["trainingTrack"],
    ...
    class GetAllBatchesByQuery(Resource):
        ...
        def get(self, trainer_id):
            ...
            if year is not None:
                try:
                    batches = BatchServices.get_all_batches_by_year(
                        int(trainer_id), int(year))
                except ValueError:
                    return INVALID_ID_ERROR, 400  # Bad Request
                ...
                try:
                    batches = BatchServices.search_for_batch(
                        int(trainer_id), track)
                except ValueError:
                    return INVALID_ID_ERROR, 400  # Bad Request
                batches_as_json = convert_list_to_json(batches)
                return batches_as_json
