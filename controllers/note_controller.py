...
def route(api: Api):
    ...
    class UnparamterizedNotes(Resource):
        @api.expect(note_data)
        @api.marshal_with(note_data, mask=None)
        ...
        @api.marshal_with(note_data, as_list=True, mask=None)
        @api.response(200, 'OK')
        def get(self):
            """ Gets all notes"""
    ...
    @api.param('note_id', 'Note Unique Id', fields.Integer)
    @api.response(200, 'OK')
    ...
    @api.route("/associates/<int:associate_id>/notes")
    @api.param('associate_id', 'Associate Unique Id')
